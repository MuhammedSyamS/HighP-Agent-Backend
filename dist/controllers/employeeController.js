...
exports.getDashboardOverview = exports.deleteEmployee = exports.updateEmployee = exports.createEmployee = exports.getEmployeeById = exports.getEmployees = void 0;
...
const getEmployees = async (req, res, next) => {
    try {
        ...
        const profiles = await EmployeeProfile_1.EmployeeProfile.find(query)
            .populate({
            path: 'userId',
...
const getEmployeeById = async (req, res, next) => {
    try {
        ...
        if (req.user?.role === shared_1.UserRole.EMPLOYEE) {
            if (!req.user.employeeProfileId || req.user.employeeProfileId.toString() !== id) {
                throw new errorHandler_1.AppError('Forbidden: Employees can only view their own profile', 403);
            }
        }
        ...
        if (profile.currentSessionId) {
            currentSession = await AttendanceSession_1.AttendanceSession.findById(profile.currentSessionId).lean();
        }
        ...
        const topApps = await ApplicationUsage_1.ApplicationUsage.find({
            companyId: req.companyId,
            employeeId: profile._id,
            date: todayStr
        })
...
const createEmployee = async (req, res, next) => {
    try {
        ...
        const existingCode = await EmployeeProfile_1.EmployeeProfile.findOne({
            companyId: req.companyId,
            employeeCode: employeeCode.trim()
        });
        if (existingCode) {
            throw new errorHandler_1.AppError(`Employee code '${employeeCode}' is already taken in your company.`, 409);
        }
        // Create User
        ...
        const profile = await EmployeeProfile_1.EmployeeProfile.create({
            ...
            ...(managerId && { managerId: new mongoose_1.default.Types.ObjectId(managerId) })
        });
        ...
        await (0, auditService_1.logAudit)({
            ...
            action: shared_1.AuditAction.EMPLOYEE_CREATED,
            resource: 'Employee',
...
const updateEmployee = async (req, res, next) => {
    try {
        ...
        const profile = await EmployeeProfile_1.EmployeeProfile.findOne({ _id: id, companyId: req.companyId });
        if (!profile) {
            throw new errorHandler_1.AppError('Employee not found', 404);
        }
        ...
        if (managerId !== undefined) {
            profile.managerId = managerId ? new mongoose_1.default.Types.ObjectId(managerId) : undefined;
        }
        await profile.save();
        // Update corresponding User record
        const user = await User_1.User.findById(profile.userId);
        if (user) {
        ...
        await (0, auditService_1.logAudit)({
            companyId: req.companyId,
            userId: req.user?.userId,
            action: shared_1.AuditAction.EMPLOYEE_UPDATED,
            resource: 'Employee',
...
const deleteEmployee = async (req, res, next) => {
    try {
        ...
        await User_1.User.updateOne({ _id: profile.userId }, { status: shared_1.UserStatus.INACTIVE });
        await (0, auditService_1.logAudit)({
            companyId: req.companyId,
            userId: req.user?.userId,
            action: shared_1.AuditAction.EMPLOYEE_DELETED,
            resource: 'Employee',
            details: { employeeId: profile._id },
            ipAddress: req.ip
        });
...
const getDashboardOverview = async (req, res, next) => {
    try {
        ...
        const profiles = await EmployeeProfile_1.EmployeeProfile.find({ companyId }).lean();
        ...
        for (const p of profiles) {
            if (p.currentStatus === shared_1.ActivityState.ACTIVE)
                activeNow++;
            else if (p.currentStatus === shared_1.ActivityState.IDLE)
                idleNow++;
            else if (p.currentStatus === shared_1.ActivityState.BREAK)
                onBreakNow++;
            else
                offlineNow++;
